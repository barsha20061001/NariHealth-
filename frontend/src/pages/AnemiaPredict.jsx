...
const features = ["Gender", "Hemoglobin", "MCH", "MCHC", "MCV"];
const sampleValues = [1, 14.9, 22.7, 29.1, 83.7];
...
export default function AnemiaPredict() {
  ...
  const fillSample = () => setValues(sampleValues);
  ...
  const predict = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8080/predict/anemia", {
        method: "POST",
        headers: {
    ...
    <div className="relative min-h-screen bg-pink-50 px-6 pt-6 pb-10">
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-[2rem] shadow-xl p-8 border border-pink-200 mt-10">
        ...
        <p className="mt-3 text-slate-600">
          Enter blood test values to get an AI-based anemia screening result.
        </p>

        <div className="mt-6 flex gap-4">
          <button onClick={fillSample} className="bg-purple-600 text-white px-5 py-3 rounded-xl font-bold">
            Fill Sample Values
          </button>

          <button onClick={predict} className="bg-pink-600 text-white px-5 py-3 rounded-xl font-bold">
        ...
        {result && (
          <div className="mt-8 rounded-2xl p-6 border bg-purple-50">
            ...
            <p className="mt-2">Prediction: <b>{result.prediction}</b></p>
